...
def ___fetch_keys_from_path(config):
    try:
        ...
        langsmith_key = config["API_KEYS"]["LANGCHAIN_API_KEY"]
    except KeyError as e:
        raise KeyError(f"Missing expected key in secrets.ini: {e}")

    return congress_key, openai_key, gai_key, gpo_key, langsmith_key

    

def _get_cdg_client():
    congress_key, _, _, _, _ = __get_api_keys()
...
def _get_oai_client():
    _, openai_key, _, _, _ = __get_api_keys()
...
def _get_gai_client():
    _, _, gai_key, _, _ = __get_api_keys()
    gai_client = genai.Client(api_key=gai_key)
    return gai_client

def _get_gpo_client():
    _, _, _, gpo_key, _ = __get_api_keys()
