 ...
 * - Agent selection dropdown (research participants only)
...
import { RESEARCH_PARTICIPANTS, getAgentDisplayName } from '../../constants/agents'
...
export function UserInput({
  ...
  const handleSelectAgent = (agent: AgentName): void => {
    onSelectAgent(agent)
  ...
  return (
    <div className="border-t border-dark-border p-4 bg-dark-bg">
      ...
      <div className="mb-3 relative">
        ...
        {isDropdownOpen && isInterrupted && (
          <div className="absolute z-10 w-full mt-1 bg-dark-hover border border-dark-border rounded shadow-lg max-h-48 overflow-y-auto">
            {RESEARCH_PARTICIPANTS.map((agent) => (
              <button
                key={agent}
      ...
      <div className="mt-2 text-xs text-gray-600">
        {isInterrupted
          ? 'Select an agent and type your message to continue the research'
