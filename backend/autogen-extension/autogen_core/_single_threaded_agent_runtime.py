...
from typing import Any, Awaitable, Callable, Dict, List, Literal, Mapping, ParamSpec, Set, Type, TypeVar, cast
...
class PendingWork:
    envelope_id: str
    kind: Literal["send", "publish", "response"]
    origin: AgentId | None
    target: AgentId | TopicId | None
    cancellation_token: CancellationToken | None
    task: asyncio.Task[Any] | None = None
    created_at: float = field(default_factory=time.time)
    status: Literal["queued", "running", "cancelled", "done"] = "queued"
...
class SingleThreadedAgentRuntime(AgentRuntime):
    ...
    def __init__(
        ...
        *,
        ...
        self._pending_work: Dict[str, PendingWork] = {}
    ...
    async def _register_pending_work(self, metadata: PendingWork) -> None:
        async with self._pending_lock:
            metadata.status = "queued"
            metadata.task = None
            self._pending_work[metadata.envelope_id] = metadata
            self._cancelled_ids.discard(metadata.envelope_id)

    async def _finalize_pending_work(self, envelope_id: str, status: Literal["done", "cancelled"]) -> None:
        async with self._pending_lock:
            work = self._pending_work.get(envelope_id)
            if work is not None:
                work.status = status
                work.task = None
                self._pending_work.pop(envelope_id, None)
            self._cancelled_ids.discard(envelope_id)

    async def _mark_pending_work_running(self, envelope_id: str, task: asyncio.Task[Any]) -> None:
        async with self._pending_lock:
            work = self._pending_work.get(envelope_id)
            if work is not None:
                work.task = task
    ...
    def _on_task_done(self, task: asyncio.Task[Any], *, envelope_id: str) -> None:
        status: Literal["done", "cancelled"] = "cancelled" if task.cancelled() else "done"
        asyncio.create_task(self._finalize_pending_work(envelope_id, status))
    ...
    async def send_message(
        ...
        *,
        ...
        with self._tracer_helper.trace_block(
            ...
            await self._register_pending_work(work)
    ...
    async def _process_send(self, message_envelope: SendMessageEnvelope) -> None:
        with self._tracer_helper.trace_block("send", message_envelope.recipient, parent=message_envelope.metadata):
            ...
            response_envelope_id = (
                f"{message_envelope.message_id}:response"
                if message_envelope.message_id is not None
                else str(uuid.uuid4())
            )
            response_work = PendingWork(
                envelope_id=response_envelope_id,
                kind="response",
                origin=message_envelope.recipient,
                target=message_envelope.sender,
                cancellation_token=None,
            )
            await self._register_pending_work(response_work)
            ...
            await self._message_queue.put(
                ResponseMessageEnvelope(
                    ...
                    metadata=get_telemetry_envelope_metadata(),
                    message_id=response_envelope_id,
                )
            )
            self._message_queue.task_done()
    ...
    async def _process_next(self) -> None:
        ...
        envelope_id = getattr(message_envelope, "message_id", None)
        if envelope_id is not None:
            async with self._pending_lock:
                metadata = self._pending_work.get(envelope_id)
                cancelled = envelope_id in self._cancelled_ids or (
                    metadata is not None and metadata.status == "cancelled"
                )
            if cancelled:
                match message_envelope:
                    case SendMessageEnvelope(future=future):
                        if not future.done():
                            future.cancel()
                    case ResponseMessageEnvelope(future=future):
                        if not future.done():
                            future.cancel()
                self._message_queue.task_done()
                await self._finalize_pending_work(envelope_id, "cancelled")
                return
        ...
        match message_envelope:
            case SendMessageEnvelope(message=message, sender=sender, recipient=recipient, future=future):
                    ...
                    for handler in self._intervention_handlers:
                        with self._tracer_helper.trace_block(
                            ...
                            try:
                                ...
                                if envelope_id is not None:
                                    await self._finalize_pending_work(envelope_id, "done")
                                return
                ...
                if envelope_id is not None:
                    await self._mark_pending_work_running(envelope_id, task)
                    task.add_done_callback(functools.partial(self._on_task_done, envelope_id=envelope_id))
            ...
            case ResponseMessageEnvelope(
                    ...
                    for handler in self._intervention_handlers:
                        try:
                            ...
                            if response_id is not None:
                                await self._finalize_pending_work(response_id, "done")
                            return
                        if temp_message is DropMessage or isinstance(temp_message, DropMessage):
                            event_logger.info(
                                MessageDroppedEvent(
                ...
                if response_id is not None:
                    await self._mark_pending_work_running(response_id, task)
                    task.add_done_callback(functools.partial(self._on_task_done, envelope_id=response_id))
                self._background_tasks.add(task)
                task.add_done_callback(self._background_tasks.discard)
    ...
    async def interrupt(
        self,
        *,
        initiator: AgentId | None = None,
        reason: str | None = None,
        include_publish: bool = True,
    ) -> None:
        tasks_to_wait: List[asyncio.Task[Any]] = []
        async with self._pending_lock:
            queued: List[PendingWork] = []
            running: List[PendingWork] = []
            for work in self._pending_work.values():
                if initiator is not None and work.origin == initiator:
                    continue
                if not include_publish and work.kind == "publish":
                    continue
        ...
        if queued or running:
            ...
            logger.info(
                "Interrupt requested%s%s: cancelled queued=%d running=%d include_publish=%s",
    ...
    async def pending_work_snapshot(self) -> List[PendingWork]:
        async with self._pending_lock:
            return [replace(work) for work in self._pending_work.values()]
