...
from ._events import UserInterrupt, UserDirectedMessage
...
class BaseGroupChat(Team, ABC, ComponentBase[BaseModel]):
    ...
    async def interrupt(self) -> None:
        """Send a USER_INTERRUPT to the manager and wait for termination sentinel.

        Ensures runtime is started to deliver the interrupt even if no active run_stream consumer.
        Runtimes exposing an `interrupt` coroutine have queued and running work cancelled
        before waiting on termination events.
        """
        if not self._initialized:
            await self._init(self._runtime)
        # Ensure runtime is running so the manager can receive the interrupt
        if self._embedded_runtime:
            assert isinstance(self._runtime, SingleThreadedAgentRuntime)
            try:
                self._runtime.start()
        ...
        interrupt_runtime = getattr(self._runtime, "interrupt", None)
        if callable(interrupt_runtime):
            await interrupt_runtime(
                initiator=AgentId(type=self._group_chat_manager_topic_type, key=self._team_id)
            )
        # Await termination sentinel or timeout
        try:
            while True:
                message_future = asyncio.ensure_future(self._output_message_queue.get())
                message = await asyncio.wait_for(message_future, timeout=2.0)
                if isinstance(message, GroupChatTermination):
                    break
            ...
            if self._embedded_runtime:
                ...
                try:
                    ...
                    pass
            # Drain any remaining queued messages to avoid leaks
            while not self._output_message_queue.empty():
                self._output_message_queue.get_nowait()
    ...
    async def send_user_message(self, msg: TextMessage, agent_name: str, trim_up: int = 0) -> TaskResult:
        ...
        - the target participant responds with a BaseChatMessage, or
        - a GroupChatTermination is received.
        """
        if not self._initialized:
            await self._init(self._runtime)

        # Ensure runtime is running so the manager can receive the request
        if self._embedded_runtime:
            assert isinstance(self._runtime, SingleThreadedAgentRuntime)
            try:
                self._runtime.start()
        ...
        # Collect messages until termination or targeted response received.
        output_messages: List[BaseAgentEvent | BaseChatMessage] = []
        stop_reason: str | None = None
        target_replied: bool = False
        while True:
            message_future = asyncio.ensure_future(self._output_message_queue.get())
            try:
                message = await asyncio.wait_for(message_future, timeout=3.0)
            except asyncio.TimeoutError:
                stop_reason = "USER_MESSAGE_TIMEOUT"
                break
            if isinstance(message, GroupChatTermination):
                if message.error is not None:
                    raise RuntimeError(str(message.error))
            ...
            if isinstance(message, ModelClientStreamingChunkEvent):
                continue
            output_messages.append(message)
            if isinstance(message, BaseChatMessage) and message.source == agent_name:
                target_replied = True
                break
        if target_replied and stop_reason is None:
            stop_reason = "USER_MESSAGE_COMPLETED"
