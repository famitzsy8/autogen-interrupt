...
Keep your responses SHORT (1-2 sentences max). Never say TERMINATE. Try to moderate between extremes."""
    ...
    # NEW: Moderate right-wing agent
    moderate_right_agent = AssistantAgent(
        name="Moderate_Right",
        model_client=model_client,
        description="Moderate center-right supporter with business-friendly but reasonable views",
        system_message="""You are a moderate center-right supporter in Chilean politics. You believe in:
- Free market principles but with some regulation
- Fiscal responsibility and gradual economic reform
- Traditional values but respect for democratic pluralism
- Business-friendly policies with social safety nets

You are more reasonable than the radical Kast supporter, seeking practical governance. You often say things like:
- "We need economic growth, but done responsibly"
- "I understand both perspectives here"
- "Let's find solutions that help business AND workers"
- "Stability and gradual progress are key"

Keep your responses SHORT (1-2 sentences max). Never say TERMINATE. Try to bridge divides with reasonable proposals."""
    )
    
    # Create team with updated termination condition
    team = RoundRobinGroupChat(
        participants=[communist_agent, liberal_agent, neural_agent, moderate_left_agent, moderate_right_agent],
        termination_condition=MaxMessageTermination(max_messages=20),  # Increased for 5 agents
    )
...
async def run_simple_interrupt_test(team, user_control, initial_topic, provocative_messages):
    ...
    print("Participants: Jara Supporter | Kast Supporter | Neural Agent | Moderate Left | Moderate Right")
    print("-" * 40)
    ...
    print("🤝 Moderate agents will seek compromise and middle ground")
    ...
    async for message in stream:
        if hasattr(message, 'content') and hasattr(message, 'source'):
            ...
            if message.source == "Neural_Agent":
                ...
                print(f"[{total_message_count}] [🤝 {message.source}]: {message.content}")
            elif message.source == "Moderate_Right":
                print(f"[{total_message_count}] [🤝 {message.source}]: {message.content}")
            else:
                print(f"[{total_message_count}] [{message.source}]: {message.content}")
            print("-" * 40)
                ...
                for msg in result.messages:
                    if hasattr(msg, 'content') and hasattr(msg, 'source') and msg.source != "RoundRobinGroupChatManager":
                        ...
                        if msg.source == "Neural_Agent":
                            ...
                            print(f"[{total_message_count}] [🤝 {msg.source}]: {msg.content}")
                        elif msg.source == "Moderate_Right":
                            print(f"[{total_message_count}] [🤝 {msg.source}]: {msg.content}")
                        else:
                            print(f"[{total_message_count}] [{msg.source}]: {msg.content}")
                        print("-" * 40)
        ...
        if total_message_count >= 50:  # Increased since we have 5 agents now
    ...
    print("🤝 Moderate agents provided centrist perspectives and compromise solutions")
    print()
    ...
    print("\n✅ Extended interrupt functionality test completed!")
