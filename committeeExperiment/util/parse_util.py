...
def _parse_committee_report_text_links(text_items):
    """
    Helper to parse and deduplicate text links from committee report text endpoint.
    Keeps all unique .htm URLs, and only one .pdf URL (the first encountered).
    """
    seen_htm = set()
    pdf_url = None
    parsed = []
    for item in text_items:
        ...
        typ = (item.findtext('type') or '').strip()
        ...
        if url.endswith('.pdf'):
            ...
            if url not in seen_htm:
                ...
                parsed.append({'url': url, 'type': typ, 'isErrata': is_errata, 'text': text})
    return parsed
