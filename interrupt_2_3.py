"""
Enhanced AutoGen Interrupt Test with Queue Content Inspection

This version shows the queue contents whenever join() is called during interrupts,
implementing the inspection at the highest possible level in the stack.
"""
...
async def test_interrupt_with_real_agents():
    """Test interrupt functionality with real OpenAI agents and queue inspection."""
    
    print("🧪 AutoGen Interrupt Functionality Test with Queue Inspection")
    print("Testing with real OpenAI agents discussing Chilean politics")
    print("=" * 60)
    
    # Apply patches before creating any teams
    #patch_all_queue_joins()
...
- Pragmatic solutions that balance social justice with economic stability
- Democratic institutions and compromise

You are more diplomatic than the radical Jara supporter, seeking middle ground. You often say things like:
- "Both sides have valid points, but..."
- "We need practical solutions that work for everyone"
- "Maybe we can find a compromise here"
- "Let's focus on what we can actually achieve"

Keep your responses SHORT (1-2 sentences max). Never say TERMINATE. Try to moderate between extremes."""
    )
    
    # Create team with updated termination condition
    team = RoundRobinGroupChat(
        participants=[communist_agent, liberal_agent, neural_agent, moderate_left_agent],
    ...
    # Run the test
    try:
        print("🚀 Starting Real Agent Interrupt Test with Queue Inspection")
        print("=" * 60)
...
async def run_simple_interrupt_test(team, user_control, initial_topic, provocative_messages):
    """Test with one continuous stream, interrupting at specific message counts with queue inspection."""
    
    print("🏛️ Starting Chilean Politics Discussion with Queue Inspection...")
    print("Participants: Jara Supporter | Kast Supporter | Neural Agent | Moderate Left")
    print("-" * 40)
    print("ℹ️  Strategy: Use one continuous stream, interrupt every ~4 messages")
    print("🔍 Queue contents will be shown when join() is called during interrupts")
    print("🤖 Neural Agent will ask questions and try to calm the debate")
    print("🤝 Moderate Left agent will seek middle ground")
    print("⏱️  Measuring interrupt times for performance analysis")
    print("-" * 40)
    
    total_message_count = 0
    provocation_index = 0
    next_provocation_at = 4  # First interrupt after 4 messages
        ...
        if hasattr(message, 'content') and hasattr(message, 'source'):
            ...
            if total_message_count >= next_provocation_at and provocation_index < len(provocative_messages) + 1:
                ...
                if provocation_index == 0:
                    ...
                    print(f"\n🛑 FIRST INTERRUPT WITH QUEUE INSPECTION (after {total_message_count} messages)!")
                    ...
                    next_provocation_at = total_message_count + 6  # Next provocation in 6 messages
                    
                else:
                    # Regular provocations - still target only the original two agents
                    provocation = provocative_messages[provocation_index - 1]
                    print(f"\n💥 PROVOCATION WITH QUEUE INSPECTION #{provocation_index} (after {total_message_count} messages)!")
    ...
    print("🤝 Moderate Left agent provided centrist perspectives")
    print()
    ...
    print("\n🔍 Queue inspection functionality test completed!")
    print("✅ Enhanced interrupt with queue content display working!")
