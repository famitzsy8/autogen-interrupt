...
async def test_interrupt_with_real_agents():
    """Test interrupt functionality with real OpenAI agents discussing Chilean politics - 4 AGENTS."""
    
    print("🧪 AutoGen Interrupt Functionality Test (4 AGENTS)")
...
Keep your responses SHORT (1-2 sentences max). Never say TERMINATE. Try to moderate between extremes."""
    ...
    # NOTE: Moderate_Right removed for 4-agent test
    ...
    team = RoundRobinGroupChat(
        ...
        termination_condition=MaxMessageTermination(max_messages=16),  # 4 agents
    )
    ...
    # Run the test
    try:
        print("🚀 Starting Real Agent Interrupt Test (4 AGENTS)")
        print("=" * 60)
...
async def run_simple_interrupt_test(team, user_control, initial_topic, provocative_messages):
    ...
    print("-" * 40)
    print("ℹ️  Strategy: Use one continuous stream, interrupt every ~8 messages")
    ...
    async for message in stream:
        if hasattr(message, 'content') and hasattr(message, 'source'):
            ...
            if total_message_count >= next_provocation_at and provocation_index < len(provocative_messages) + 1:
                ...
                if provocation_index == 0:
                    ...
                    next_provocation_at = total_message_count + 8  # Next provocation in 8 messages
        ...
        if total_message_count >= 32:  # Reasonable for 4 agents
    ...
    print("\n✅ 4-Agent interrupt functionality test completed!")
